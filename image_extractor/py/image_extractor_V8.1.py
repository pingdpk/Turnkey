        ...
        with open(inputFilePath, 'r') as inputTextFile:
            for line in chunked_read(inputTextFile, input_file_new_line):
                if line.rstrip():
                    ...
                    #sengul:with open(outputFilePath, 'a') as outputTextFile: is changed as below for utf-8 conversion
                    with codecs.open(outputFilePath, "a","utf-8") as outputTextFile:
                        outputTextFile.write(output)

                    closeOpenedFile(outputTextFile)

        #sengul: following 3 lines are added. There is missing end at the end of file
        outputTextFile=codecs.open(outputFilePath, "a","utf-8")
